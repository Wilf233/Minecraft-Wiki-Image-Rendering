...
h, s, v = durability / max_durability * 120, 1, 1
r, g, b = hsv_to_rgb(h, s, v)

for filename in os.listdir(folder_path):
    file_path = os.path.join(folder_path, filename)

    if durability < max_durability and durability >= 0:
        if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
            try:
                ...
                if round(durability / max_durability * 13) >= 1:
                    draw.rectangle([(4, 26), (3 + 2 * round(durability / max_durability * 13), 27)], fill=(r, g, b, 255))
